...
Error_train_i = np.empty((K,1))
Error_test_i = np.empty((K,1))
...
for train_index, test_index in CV.split(X, y):
    ...
    # -----feature selection-----
    textout = ''
    selected_features, features_record, loss_record = feature_selector_lr(X_train_fs, y_train, display=textout)
    ...
    opt_var_err, opt_lambda, mean_w_lambda, train_err_lambda, test_err_lambda = rlr_validate(X_train, y_train, lambdas)
    '''
    for lambda_e in lambdas:
        lambdaI = lambda_e*np.eye(M)
        lambdaI[0,0] = 0
        w_reg[:, k] = np.squeeze(np.linalg.solve((X_train.T @ X_train + lambdaI), (X_train.T @ y_train)))
        Error_train_i[k] = np.sum(np.square(y_train - X_train @ w_reg[:, k]), axis=0) / np.shape(y_train)[0]
        Error_test_i[k] = np.sum(np.square(y_test - X_test @ w_reg[:, k]), axis=0) / np.shape(y_test)[0]
    
    opt_lambda = lambdas[np.unravel_index(np.argmin(Error_test_i), Error_test_i.shape)[1]]
