...
for(k, (train_index, test_index)) in enumerate(CV.split(X, y)):
    ...
    for (j, (train_index_i, test_index_i)) in enumerate(CV.split(X_train, y_train)):
        ...
        for n_hidden_units in n_hidden_units_l:
            ...
            '''
            h, = summaries_axes[0].plot(learning_curve, color=color_list[j-1])
            h.set_label('CV fold K1: {0}, K2: {1}, nhu: {2}'.format(k+1, j, n_hidden_units))
