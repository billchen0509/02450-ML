...
for(k, (train_index, test_index)) in enumerate(CV.split(X, y)):
    ...
    z_lr = np.abs(y_test_lr - (X_test_lr @ w_reg[:, k]).reshape((len(y_test_lr), 1)))**2
    ...
    z_ann = se
    mse = (sum(se).type(torch.float)/len(y_test_i)).data.numpy()
    errors.append(round(mse[0],4))
    
    
    # ----------baseline----------
    baseline.append(round(np.sum(y[test_index])/len(y[test_index]), 4))
